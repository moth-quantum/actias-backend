<script lang="ts">
    ...
    function timeout() {
        if (--counter > 0 && show) return setTimeout(timeout, 1000);
        show = false;
        onClose();
    }
...
{#if type === 'success'}
    <Toast color="green" class="mb-2" bind:open={show} on:close={() => show = false}>
...
{:else if type === 'error'}
    <Toast color="red" class="mb-2" bind:open={show} on:close={() => show = false}>
        <svelte:fragment slot="icon">
...
{:else if type === 'warning'}
    <Toast color="pink" class="mb-2" bind:open={show} on:close={() => show = false}>
        <svelte:fragment slot="icon">
